    ...
    def influence_vector(self, flag=0):
        ...
        if (FLAGS.encrypt):
            ...
            for num in [5]:
                ...
                # sp.save_npz("temp/%s_logist1_train_poison_data_%d_%d_%d.npz" % (
                #     FLAGS.dataset, num, FLAGS.encrypt, FLAGS.decrypt),
                #             sp.csr_matrix(train_poison_data))
                # sp.save_npz("temp/%s_logist1_train_poison_label_%d_%d_%d.npz" % (
                #     FLAGS.dataset, num, FLAGS.encrypt, FLAGS.decrypt),
                #             sp.csr_matrix(train_poison_label))
            # for num in [1, 2, 3, 4, 5, 6, 7, 8]:
            #     train_poison_data = sp.load_npz(
            #         "temp/%s_logist1_train_poison_data_%d_%d_%d.npz" % (
            #             FLAGS.dataset, num * 2, FLAGS.encrypt, FLAGS.decrypt)).toarray()
            #     train_poison_data = utils.step2(train_poison_data, FLAGS.dataset, num)
            #     sp.save_npz("temp/%s_logist2_train_poison_data_%d_%d_%d.npz" % (
            #         FLAGS.dataset, num, FLAGS.encrypt, FLAGS.decrypt),
            #                 sp.csr_matrix(train_poison_data))
            #     sp.save_npz("temp/%s_logist2_train_poison_label_%d_%d_%d.npz" % (
            #         FLAGS.dataset, num, FLAGS.encrypt, FLAGS.decrypt),
            #                 sp.csr_matrix(train_poison_label))
        else:
            print("decrypt")
