...
class MaxEnt:
    ...
    def create_d_model(self, input):
        output = input
        for weight, bias in zip(self.d_weights[:-1], self.d_biases[:-1]):
            output = tf.matmul(output, weight) + bias
            output = tf.nn.relu(output)
    ...
    def create_optimizer(self):
        ...
        self.d_loss = tf.reduce_mean(tf.square(self.res_input - self.adv_input))
        ...
        if (FLAGS.dataset == 'app'):
            ...
            self.optimizer = tf.train.AdamOptimizer(0.0001)
        self.step1_loss = self.a_loss + self.e_loss + FLAGS.reg * (self.reg_a_loss + self.reg_e_loss)
        self.step2_loss = self.d_loss + self.e_loss + FLAGS.reg * (self.reg_a_loss + self.reg_e_loss)
        self.train_a_op = self.optimizer.minimize(self.step1_loss, name='optimizer1',
                                                  var_list=self.e_weights + self.e_biases + self.a_weights + self.a_biases)
        self.train_d_op = self.optimizer.minimize(self.step2_loss, name='optimizer2',
                                                  var_list=self.e_weights + self.e_biases + self.d_weights + self.d_biases)
    ...
    def train(self, epochs, batch_size):
        ...
        for i in range(epochs):
            ...
            for cur_x, cur_y in batchs:
                for e in range(3):
                    step1, _ = self.sess.run([self.step1_loss, self.train_a_op],
                                             feed_dict={self.input: cur_x, self.label: cur_y})

                cur_y = np.ones_like(cur_y) * 1 / (cur_y.shape[1])
                for e in range(3):
                    step2, _ = self.sess.run([self.step2_loss, self.train_d_op],
                                             feed_dict={self.input: cur_x, self.label: cur_y})
                step1_list.append(step1)
                step2_list.append(step2)
            print('epochs %d:' % i, np.mean(step1_list), np.mean(step2_list))
        for i in range(5, 6):
            FLAGS.num = i
            self.generate_noise()
        # self.generate_noise()
    ...
    def generate_noise(self):
        ...
        for cur_x, cur_y in batches:
            ...
            cur_noise = self.sess.run(self.adv_input, feed_dict={self.input: cur_x})
        ...
        # sp.save_npz("temp/%s_maxent_test_poison_data_%d_%d_%d.npz" % (
        #     FLAGS.dataset, FLAGS.num, FLAGS.encrypt, FLAGS.decrypt),
        ...
        for cur_x, cur_y in batches:
            ...
            # break
        noises = np.concatenate(noise_list)
        # sp.save_npz("temp/%s_maxent_train_poison_data_%d_%d_%d.npz" % (
        #     FLAGS.dataset, FLAGS.num, FLAGS.encrypt, FLAGS.decrypt),
        #             sp.csr_matrix(noises))
