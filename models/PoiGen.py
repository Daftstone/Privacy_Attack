...
class PoiGen:
    ...
    def generate_noise(self):
        ...
        for cur_x, cur_y in batches:
            ...
            for i in range(5):
                ...
                if (FLAGS.dataset == 'weibo'):
                    cur_grad[:, :4002] = 0
                cur_grad = cur_grad / (np.sum(np.abs(cur_grad), axis=1, keepdims=True) + 1e-10)
                g = g + cur_grad
                adv = cur_noise + 0.2 * np.sign(g)
                delta = np.clip(adv - cur_noise, -0.2, 0.2)
                cur_noise = np.clip(cur_noise + delta, 0, 1)
        ...
        for num in range(5, 6):
            ...
            noises = np.concatenate(noise_all)
            noises[noises > 0] = 1.
            if (FLAGS.dataset == 'app'):
