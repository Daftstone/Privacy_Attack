...
class Adv_train:
    ...
    def create_optimizer(self):
        ...
        self.gradient = tf.gradients(self.loss + self.reg_loss * 1e-4, self.input)
    ...
    def train(self, epochs, batch_size):
        ...
        for i in range(epochs):
            ...
            for cur_x, cur_y in batchs:
                gradient = self.sess.run(self.gradient, feed_dict={self.input: cur_x, self.label: cur_y})
                gradient = np.sign(gradient) * 0.005
                self.sess.run(self.train_op,
                              feed_dict={self.input: cur_x + gradient, self.label: cur_y, self.training: True})
